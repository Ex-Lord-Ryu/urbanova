        ...
        <form action="{{ route('shop') }}" method="GET" id="filterForm">
          ...
          <div class="filter-group">
            ...
            <ul>
              <li>
                <label>
                  <input type="checkbox" name="discount" value="1" {{ request()->has('discount') ? 'checked' : '' }}>
                  On Sale
                </label>
              </li>
            </ul>
  ...
  $('.btn-add-to-cart').click(function() {
    ...
    Swal.fire({
      ...
      preConfirm: () => {
        ...
        return { size: selectedSize, qty: qty };
      },
      didOpen: () => {
      ...
      if (result.isConfirmed && result.value) {
        $.ajax({
          ...
          error: function(xhr) {
            Swal.fire('Error', xhr.responseJSON?.message || 'Gagal menambah ke cart', 'error');
          }
        });
      }
    });
