...
@push('css')
    ...
    <style>
        /* Add any additional styles that are not in shop.css here */
        .btn-color:hover {
            transform: scale(1.1);
        }
                ...
                <form action="{{ route('shop') }}" method="GET" id="filterForm">
                    ...
                    <div class="filter-group search-box">
                        ...
                        <div class="search-input-wrapper">
                            <input type="text" name="search" id="search" placeholder="Find products..." value="{{ request('search') }}" class="search-input">
                        ...
                        @if(request('search'))
                            <div class="active-search">
                                <span>Searching for: <strong>"{{ request('search') }}"</strong></span>
                            </div>
                        @endif
                    </div>
                            ...
                            @foreach ($categories as $category)
                                <li>
                                    <label>
                                        ...
                                        {{ $category->name }}
                                        <span
                                            class="count">({{ $category->products->whereIn('id', $productsWithStock)->count() }})</span>
                                    </label>
                                </li>
                            @endforeach
                        </ul>
                                ...
                                <li>
                                    <label>
                                        <input type="checkbox" name="size[]" value="{{ $size->id }}"
                                            {{ in_array($size->id, is_array($selectedSizes) ? $selectedSizes : []) ? 'checked' : '' }}>
                                        {{ $size->name }}
                                        <span
                                            class="count">({{ $size->productVariants->where('stock', '>', 0)->count() }})</span>
                                    </label>
                                </li>
                            @endforeach
                        </ul>
                            ...
                            @foreach ($colors as $color)
                                <li class="color-filter-item">
                                    <input type="checkbox" id="color-{{ $color->id }}" name="color[]"
                                        value="{{ $color->id }}"
                                        {{ in_array($color->id, is_array($selectedColors) ? $selectedColors : []) ? 'checked' : '' }}>
                                    <span class="color-circle"
                                        style="display: inline-block; width: 20px; height: 20px; border-radius: 50%; background-color: #{{ $color->hex_code }}; border: 1px solid #ddd; margin: 0 8px;"></span>
                                    <label for="color-{{ $color->id }}">
                                        {{ $color->name }}
                                        <span
                                            class="count">({{ $color->productVariants->where('stock', '>', 0)->count() }})</span>
                                    </label>
                                </li>
                            @endforeach
                        </ul>
                    ...
                    <div class="filter-group">
                        ...
                        <p class="mt-2 mb-1">Or select a range:</p>
                        <ul>
                            @forelse($priceRanges as $index => $range)
                                <li>
                                    <label>
                                        <input type="radio" name="price" value="{{ $index }}"
                    ...
                    <a href="{{ route('shop') }}" class="reset-filters">RESET FILTERS</a>
                </form>
            </aside>
                    ...
                    <div class="product-card {{ $product->is_featured && $showFeaturedBadge ? 'featured' : '' }}">
                        ...
                        @if ($product->is_featured && $showFeaturedBadge)
                            <div class="featured-badge">
                                <i class="fas fa-star" style="color: #000; margin-right: 5px;"></i> Unggulan
                            </div>
                        @endif
                            ...
                            <div class="product-price">
                                @if ($product->discount_percentage > 0)
                                    <div class="discount-price">Rp
                                        {{ number_format($product->discounted_price, 0, ',', '.') }}</div>
...
@push('scripts')
    ...
    <script>
        $(document).ready(function() {
            ...
            if ($('#search').val()) {
                $('#search').after('<span class="search-clear">&times;</span>');
            }

            $('#search').on('input', function() {
                if ($(this).val()) {
                    if (!$('.search-clear').length) {
                        $(this).after('<span class="search-clear">&times;</span>');
                    }
                } else {
                    $('.search-clear').remove();
                }
            ...
            $('.btn-add-to-cart').click(function() {
                ...
                sizes.forEach(function(size) {
                    ...
                    const sizePrice = hasSizePrices && sizePrices[size] ? sizePrices[size] :
                        productPrice;
                    const discountedSizePrice = hasActiveDiscount ? sizePrice - (sizePrice *
                        discountPercentage / 100) : sizePrice;
          ...
          `,
                            ...
                            preConfirm: () => {
                                const selectedSize = $('.btn-size.active').data(
                                    'size');
                                const selectedColorId = $('#selected-color-id')
                                .val();
                                const qty = parseInt($('#swal-qty').val());
                                const colorContainerVisible = $(
                                    '#color-options-container').is(':visible');
                                const colorOptionsExist = $('#color-options')
                                    .children().length > 0;
                                ...
                                if (colorContainerVisible && colorOptionsExist && !
                                    selectedColorId) {
                            ...
                            didOpen: () => {
                                ...
                                function isLightColor(hexCode) {
                                    ...
                                    const brightness = ((r * 299) + (g * 587) + (b *
                                        114)) / 1000;
                                ...
                                $('.btn-size').click(function() {
                                    ...
                                    if (hasSizePrices) {
                                        const sizePrice = $(this).data(
                                            'price');
                                        if (sizePrice) {
                                            const formattedPrice =
                                                formatRupiah(sizePrice);
                                            ...
                                            if (hasActiveDiscount) {
                                                ...
                                                $('#swal-price-display')
                                                    .html(
                                                        `<del>Rp ${formattedPrice}</del> <span class="ml-2 h5" style="color:#fc544b;font-weight:700;">Rp ${formattedDiscountedPrice}</span>`
                                                        );
                                            } else {
                                                $('#swal-price-display')
                                                    .html(
                                                        `<span class="ml-2 h5" style="color:#fc544b;font-weight:700;">Rp ${formattedPrice}</span>`
                                                        );
                                            }
                                        }
                                    }
