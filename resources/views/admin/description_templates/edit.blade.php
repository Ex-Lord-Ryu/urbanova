...
@section('title', 'Edit Template Deskripsi')
...
<section class="section">
    ...
    <div class="section-body">
        <h2 class="section-title">Edit Template Deskripsi</h2>
        <p class="section-lead">
            Ubah template deskripsi produk sesuai kebutuhan Anda.
        </p>
            ...
            <div class="col-lg-8">
                <div class="card">
                    <div class="card-header">
                        <h4>Form Edit Template</h4>
                    </div>
                    <div class="card-body">
                        <form action="{{ route('description-templates.update', $descriptionTemplate->id) }}" method="POST">
                            @csrf
                            @method('PUT')
                            <div class="form-group">
                                <label for="name">Nama Template <span class="text-danger">*</span></label>
                                <input type="text" class="form-control @error('name') is-invalid @enderror"
                                    id="name" name="name" value="{{ old('name', $descriptionTemplate->name) }}" required>
                                @error('name')
                                    ...
                                    <select class="form-control @error('category') is-invalid @enderror" id="category"
                                        ...
                                        @foreach ($categories as $category)
                                            <option value="{{ $category }}"
                                                {{ old('category', $descriptionTemplate->category) == $category ? 'selected' : '' }}>{{ $category }}
                                            </option>
                                        @endforeach
                                    </select>
                            ...
                            <div class="form-group">
                                ...
                                <textarea class="form-control summernote @error('content') is-invalid @enderror" id="content" name="content" required>{{ old('content', $descriptionTemplate->content) }}</textarea>
                                @error('content')
                                <div class="invalid-feedback">
                                    {{ $message }}
                                </div>
                            ...
                            <div class="form-group">
                                <div class="custom-control custom-switch">
                                    <input type="checkbox" class="custom-control-input" id="is_active" name="is_active"
                                        value="1" {{ old('is_active', $descriptionTemplate->is_active) ? 'checked' : '' }}>
